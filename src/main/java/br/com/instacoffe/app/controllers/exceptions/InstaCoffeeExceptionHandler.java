...
public class InstaCoffeeExceptionHandler {
    ...
    public ResponseEntity<StandardResponseErrorDto> handleEntityAlreadyExistsException(HttpServletRequest request, ResourceAlreadyExistsException exception){
        StandardResponseErrorDto responseErrorDto = makeStandardResponseErrorDto(HttpStatus.BAD_REQUEST.value(), "Resource not found", exception.getMessage(), request.getRequestURI(), new HashSet<>());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(responseErrorDto);
    }


    private static StandardResponseErrorDto makeStandardResponseErrorDto(Integer status, String error, String message, String path, Set<FieldError> errors){
